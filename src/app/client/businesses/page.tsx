      ...
      {isMobile ? (
        <main className="flex-1 pb-28 w-full overflow-x-hidden relative bg-black pb-safe">
          <Topbar
            ...
            subtitle={isRefreshing ? 'Yenileniyor...' : (mall ? `${mall.name}` : "Yetkili Şubeler")}
          />
          ...
          <div className="p-4 space-y-4">
            ...
            {mall && (
              <div className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-red-500/20 to-orange-500/10 border border-border flex items-center justify-center text-primary shrink-0">
                  <Building2 size={24} />
                </div>
                <div className="min-w-0 flex-1">
                  <h1 className="text-sm font-black uppercase tracking-wider text-foreground truncate">{mall.name}</h1>
                  {mall.address && (
                    <p className="text-[10px] text-muted-foreground flex items-center gap-1 mt-0.5 truncate">
                      <MapPin size={10} className="text-red-400" /> {mall.address}
                    </p>
                  )}
                </div>
                <button 
                  onClick={() => fetchData(true)}
                  className="p-2.5 rounded-xl bg-secondary border border-border text-muted-foreground active:scale-90 transition-all shrink-0"
                >
            ...
            {loading && !initialLoadDone.current ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest text-white/30">Listeleniyor</p>
              </div>
            ) : filteredBiz.length === 0 ? (
              <div className="bg-card border border-border rounded-xl py-16 flex flex-col items-center text-center px-4">
                <Store className="w-12 h-12 text-muted-foreground/20 mb-3" />
                <h3 className="text-sm font-black uppercase tracking-wider text-foreground/80">İşletme Bulunamadı</h3>
                <p className="text-[10px] text-muted-foreground mt-1 max-w-[200px] leading-normal mx-auto">Bu AVM için henüz bir dükkan tanımlanmamış veya arama kriterine uygun sonuç yok.</p>
              </div>
            ) : (
                    ...
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-wider mt-0.5">
                          {biz.category || 'Resmi İşletme'}
                        </p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-3 shrink-0">
                      <span className="px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[8px] font-black uppercase tracking-widest">
                        Aktif
                      </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {filteredBiz.map((biz, i) => (
                  <Link
                    key={biz.id}
                    href={`/client/businesses/${biz.id}`}
                    className="glass group hover:bg-white/[0.03] transition-all duration-500 rounded-xl p-4 border border-border flex flex-col justify-between shadow-lg relative overflow-hidden"
                  ...
                  >
                    ...
                    <div className="relative space-y-4">
                      <div className="flex items-start justify-between">
                        <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-red-500/20 to-orange-500/10 border border-border flex items-center justify-center text-primary group-hover:scale-110 transition-transform">
                          <Store size={22} />
                        </div>
                    ...
                    <div className="relative mt-8 pt-4 border-t border-border flex items-center justify-between text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60 group-hover:text-red-400 transition-colors">
