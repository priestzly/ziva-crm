                  ...
                  <div ref={printRef} id="quote-preview" className="p-8 md:p-12 lg:p-16">
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-8 pb-8 border-b-2 border-slate-900">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h2 className="text-xl font-black uppercase tracking-tight">ZIVA <span className="text-red-500">BACA YANGIN</span></h2>
                          <p className="text-[9px] uppercase tracking-[0.3em] text-slate-400 font-bold">Teknik Servis & Güvenlik Sistemleri</p>
                        </div>
                      </div>
                      <div className="text-left sm:text-right">
                        <p className="text-2xl font-black text-slate-100 uppercase">{quoteData.title}</p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 py-8">
                      <div className="space-y-2">
                        <p className="text-[9px] font-black uppercase tracking-widest text-red-500">Sayın Müşteri</p>
                        <p className="text-lg font-black text-slate-900">{quoteData.clientName || '—'}</p>
                        {quoteData.clientAddress && <p className="text-xs text-slate-500">{quoteData.clientAddress}</p>}
                        {quoteData.clientPhone && <p className="text-xs text-slate-500 flex items-center gap-1"><Phone size={10} /> {quoteData.clientPhone}</p>}
                        {quoteData.clientEmail && <p className="text-xs text-slate-500 flex items-center gap-1"><Mail size={10} /> {quoteData.clientEmail}</p>}
                        {quoteData.clientTaxNo && <p className="text-xs text-slate-500 flex items-center gap-1"><Hash size={10} /> VN: {quoteData.clientTaxNo}</p>}
                      </div>
                      <div className="text-left sm:text-right space-y-2">
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-300">Geçerlilik</p>
                        <p className="text-sm font-bold text-slate-900">{new Date(quoteData.validUntil).toLocaleDateString('tr-TR')}</p>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="bg-slate-900 text-white">
                            <th className="py-3 px-4 text-center font-bold text-[9px] uppercase tracking-wider">#</th>
                            <th className="py-3 px-4 text-left font-bold text-[9px] uppercase tracking-wider">Ürün / Hizmet</th>
                            <th className="py-3 px-3 text-center font-bold text-[9px] uppercase tracking-wider">Adet</th>
                            <th className="py-3 px-3 text-right font-bold text-[9px] uppercase tracking-wider">Birim</th>
                            {items.some(i => i.discount > 0) && (
                              <th className="py-3 px-3 text-center font-bold text-[9px] uppercase tracking-wider">İnd.</th>
                            )}
                            <th className="py-3 px-4 text-right font-bold text-[9px] uppercase tracking-wider">Toplam</th>
                          </tr>
                        </thead>
                          ...
                          {items.map((item, i) => (
                            <tr key={item.id} className="bg-white">
                              ...
                              {items.some(it => it.discount > 0) && (
                                <td className="py-3 px-3 text-center text-emerald-600 font-semibold">%{item.discount}</td>
                              )}
                              <td className="py-3 px-4 text-right font-bold text-slate-900">{getItemTotal(item).toLocaleString('tr-TR')} ₺</td>
                            </tr>
                          ))}
                        </tbody>
                        <tfoot>
                          <tr className="bg-slate-50">
                            ...
                            <td className="py-3 px-4 text-right font-bold text-slate-900">{subtotal.toLocaleString('tr-TR')} ₺</td>
                          </tr>
                          {totalDiscount > 0 && (
                          ...
                          {isTaxIncluded && (
                            <tr className="bg-slate-50/50">
                              <td colSpan={3}></td>
                              <td className="py-3 px-4 text-right text-[9px] font-bold uppercase tracking-wider text-slate-400">KDV (%{taxRate})</td>
                              {items.some(i => i.discount > 0) && <td className="py-3 px-4"></td>}
                              <td className="py-3 px-4 text-right font-bold text-slate-900">{taxAmount.toLocaleString('tr-TR')} ₺</td>
                            </tr>
                          )}
                          <tr className="bg-slate-900 text-white">
                            <td colSpan={3}></td>
